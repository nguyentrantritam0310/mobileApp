...
function LeaveListContent() {
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('Leave list screen focused, reloading data...');
      loadLeaveRequests();
    }, [])
  );
  ...
  const handleEdit = (voucherCode) => {
    router.push(`/leave/edit/${voucherCode}`);
  };
  ...
  const renderLeaveItem = ({ item }) => (
    <View style={styles.card}>
      <TouchableOpacity 
        onPress={() => router.push(`/leave/${item.voucherCode}`)} 
  ...
  return (
    ...
    >
      ...
      <FlatList
        ...
        contentContainerStyle={leaveRequests.length === 0 ? styles.emptyListContainer : null}
      />
