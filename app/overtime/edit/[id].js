...
export default function EditOvertimeScreen() {
  ...
  const { getOvertimeRequestById, updateOvertimeRequest, loading: overtimeLoading } = useOvertimeRequest();
    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          overtimeFormID: data.overtimeFormID || '',
      ...
      Alert.alert('Lỗi', 'Không thể tải thông tin đơn tăng ca');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateOvertimeRequest(formData.voucherCode, submitData);
      Alert.alert('Thành công', 'Cập nhật đơn tăng ca thành công', [
      ...
      Alert.alert('Lỗi', `Không thể cập nhật đơn tăng ca: ${error.response?.data?.message || error.message}`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <CustomHeader title="Sửa đơn tăng ca" />
        ...
        <View style={styles.form}>
          ...
          >
            {submitting || overtimeLoading ? (
              ...
              <Text style={styles.submitText}>Cập nhật đơn tăng ca</Text>
            )}
          </TouchableOpacity>
        </View>
      </ScrollView>
